...
const WorkGrid = (root) => {
  ...
  const _tick = () => {
    ...
    _cursor.style.transform = `translate3d(${_current.x.toFixed(2)}px, ${_current.y.toFixed(2)}px, 0)`;
  };
  ...
  const init = () => {
    ...
    if (!isTouchDevice() && _cursor) {
      ...
      const removeMove = delegate(
        root,
        "mousemove",
        "[data-work-card]",
        _onMouseMove,
      );
      const removeEnter = delegate(
        root,
        "mouseenter",
        "[data-work-card]",
        _showCursor,
